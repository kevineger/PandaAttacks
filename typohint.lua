...
function scene:create( event )
	...
	local typoString = "Check your spelling for:\n"
	for key,value in pairs(event.params) do
		typoString = typoString .. value
		if event.params[key + 2] then
			typoString = typoString .. ", "
		elseif event.params[key + 1] then
			typoString = typoString .. " and "
		end
	end
	typoString = typoString .. "\n(Tap to dismiss)"
